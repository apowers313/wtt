...
class PathUtils {
  ...
  static normalize(inputPath) {
    ...
    if (os.platform() === 'win32') {
      console.log(`${debugPrefix} - Platform is Windows, attempting realpath`);
      try {
        // Try to resolve to the canonical path, which converts short names to long names
        const realPath = fs.realpathSync(absolutePath);
        console.log(`${debugPrefix} - realpathSync succeeded: "${realPath}"`);
        absolutePath = realPath;
      } catch (error) {
        console.log(`${debugPrefix} - realpathSync failed: ${error.message}`);
        ...
        try {
          ...
          console.log(`${debugPrefix} - Trying parent: "${parentDir}", file: "${fileName}"`);
          const realParent = fs.realpathSync(parentDir);
          absolutePath = path.join(realParent, fileName);
          console.log(`${debugPrefix} - Parent realpath succeeded: "${absolutePath}"`);
        } catch (parentError) {
          console.log(`${debugPrefix} - Parent realpath failed: ${parentError.message}`);
