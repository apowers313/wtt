...
class GitOutputParser {
  ...
  static parseWorktreeList(output) {
    console.log('[DEBUG] GitOutputParser.parseWorktreeList - Starting');
    console.log('[DEBUG] GitOutputParser.parseWorktreeList - Input:', output);
    
    if (!output || !output.trim()) {
      console.log('[DEBUG] GitOutputParser.parseWorktreeList - Empty output, returning empty array');
      return [];
    }

    // Split by line, handling both Unix and Windows line endings
    const lines = output.split(/\r?\n/).filter(line => line.trim());
    console.log('[DEBUG] GitOutputParser.parseWorktreeList - Lines count:', lines.length);
    console.log('[DEBUG] GitOutputParser.parseWorktreeList - Lines:', lines);
    
    return lines.map((line, index) => {
      console.log(`[DEBUG] GitOutputParser.parseWorktreeList - Parsing line ${index}: "${line}"`);
      ...
      if (!commitMatch) {
        console.log(`[DEBUG] GitOutputParser.parseWorktreeList - Line ${index}: No commit hash found`);
        return null;
      }
      ...
      console.log(`[DEBUG] GitOutputParser.parseWorktreeList - Line ${index} parsed:`);
      console.log('  Raw path:', worktreePath);
      ...
      console.log(`  Branch: "${branch}", bare: ${bare}, detached: ${detached}`);
