 ...
 * @param {string} mainBranch - The main branch name (defaults to 'main')
 * @returns {string} User-friendly error message
 */
function translateGitError(error, context = '', mainBranch = 'main') {
  const message = error.message || error.toString();
  
  // Branch/reference errors
  if (message.includes('fatal: invalid reference')) {
    const branch = message.match(/invalid reference: (.+)/)?.[1];
    return `The branch '${branch}' doesn't exist. Try 'wt create ${branch} --from ${mainBranch}' to create it as a new branch.`;
  }
