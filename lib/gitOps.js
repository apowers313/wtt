...
class GitOps {
  ...
  async listWorktrees() {
    console.log('[DEBUG] gitOps.listWorktrees - Starting');
    console.log('[DEBUG] gitOps.listWorktrees - CWD:', process.cwd());
    
    // Use regular list format which is more reliable than porcelain
    const output = await this.git.raw(['worktree', 'list']);
    console.log('[DEBUG] gitOps.listWorktrees - Raw git output:');
    console.log(output);
    console.log('[DEBUG] gitOps.listWorktrees - Raw output bytes:', Buffer.from(output).toString('hex'));
    
    // Use GitOutputParser for consistent parsing
    const worktrees = GitOutputParser.parseWorktreeList(output);
    console.log('[DEBUG] gitOps.listWorktrees - Parsed worktrees:', JSON.stringify(worktrees, null, 2));
