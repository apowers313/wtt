...
class GitOps {
  ...
  async listWorktrees() {
    ...
    if (process.env.CI || process.env.DEBUG_TESTS) {
      ...
      console.log('  Platform:', process.platform);
      console.log('  Raw git output:', output);
    }
  ...
  isWorktreePath(dirPath) {
    ...
    const worktreesPath = path.join('.worktrees', path.sep);
    return normalizedPath.includes(worktreesPath) || normalizedPath.includes('.worktrees' + path.sep);
  }
