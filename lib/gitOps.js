...
class GitOps {
  ...
  async createWorktree(worktreePath, branchName, baseBranch = null, mainBranch = 'main') {
    const git = await this.ensureGit();
    ...
    try {
      ...
      throw new Error(translateGitError(error, 'create worktree', mainBranch));
    }
  }
  ...
  async listWorktrees() {
    ...
    try {
      ...
      if (process.env.WTT_DEBUG === 'true') {
        console.error('[DEBUG] Git worktree list raw output:');
        console.error(output);
        console.error('[DEBUG] Running from directory:', this.mainRoot || process.cwd());
      }
  ...
  async hasUncommittedChanges(worktreePath) {
    const git = await this.ensureGit();
    try {
      // If no path provided, use the main repository root
      const targetPath = worktreePath || this.mainRoot;
      if (!targetPath) {
        // Fallback to current directory if mainRoot not set
        const output = await git.raw(['status', '--porcelain']);
        return output.trim().length > 0;
      }
      
      // Use -C flag to run git in the specified directory
      const gitPath = PathUtils.toPosix(targetPath);
